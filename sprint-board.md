...
| DRV-QA-7 | DRV-US-002/003 | Add Excel/import row-level validation tests | Backend/QA | P0 | partial | Service-level Excel-normalized row validation now has backend coverage for valid rows, duplicate IDs, invalid windows, draft vs ready routeability, and actionable row errors. Real `.xlsx` upload/parser/API coverage remains. Updated by Stage 6 QA. |
